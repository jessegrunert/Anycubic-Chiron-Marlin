...
typedef struct SettingsDataStruct {
  ...
  //  ANYCUBIC TFT
  //
  unsigned char tftSelectedLevelingMode;                   //M1000, M1001
  float         tftSelectedOffsetZ;
  float         tftPowerLossHeadE, tftPowerLossHeadX, tftPowerLossHeadY, tftPowerLossHeadZ;
  long          tftPowerLossCard;
#endif
} SettingsData;
...
void MarlinSettings::postprocess() {
  ...
  bool MarlinSettings::save() {
    ...
    //
    //  Leveling type Manual M1000, Auto = M1001 
    //

    _FIELD_TEST(tftSelectedLevelingMode);
    #if ENABLED(ANYCUBIC_CHIRON)
      EEPROM_WRITE(tftSelectedLevelingMode);
    #else
      unsigned char tftSelectedLevelingModeDummy = 0;
      EEPROM_WRITE(tftSelectedLevelingModeDummy);
    #endif

    _FIELD_TEST(tftSelectedOffsetZ);

    #if ENABLED(ANYCUBIC_CHIRON)
      EEPROM_WRITE(tftSelectedOffsetZ);
    #else
      dummy = 0.0f;
      EEPROM_WRITE(dummy);
    #endif

    _FIELD_TEST(tftPowerLossHeadE);

    #if ENABLED(ANYCUBIC_CHIRON)
      EEPROM_WRITE(tftPowerLossHeadE);
    #else
      dummy = 0.0f;
      EEPROM_WRITE(dummy);
    #endif

    _FIELD_TEST(tftPowerLossHeadX);

    #if ENABLED(ANYCUBIC_CHIRON)
      EEPROM_WRITE(tftPowerLossHeadX);
    #else
      dummy = 0.0f;
      EEPROM_WRITE(dummy);
    #endif

    _FIELD_TEST(tftPowerLossHeadY);

    #if ENABLED(ANYCUBIC_CHIRON)
      EEPROM_WRITE(tftPowerLossHeadY);
    #else
      dummy = 0.0f;
      EEPROM_WRITE(dummy);
    #endif
    
    _FIELD_TEST(tftPowerLossHeadZ);

    #if ENABLED(ANYCUBIC_CHIRON)
      EEPROM_WRITE(tftPowerLossHeadZ);
    #else
      dummy = 0.0f;
      EEPROM_WRITE(dummy);
    #endif  

    _FIELD_TEST(tftPowerLossCard);

    #if ENABLED(ANYCUBIC_CHIRON)
      EEPROM_WRITE(tftPowerLossCard);
    #else
      long tftPowerLossCardDummy = 0;
      EEPROM_WRITE(tftPowerLossCardDummy);
    #endif
  ...
  bool MarlinSettings::_load() {
    ...
    if (strncmp(version, stored_ver, 3) != 0) {
      ...
      //
      // Selected leveling type Manual = M1000, Auto = M1001
      //

      _FIELD_TEST(tftSelectedLevelingMode);
     #if ENABLED(ANYCUBIC_CHIRON)
        EEPROM_READ(tftSelectedLevelingMode);
        if (tftSelectedLevelingMode != AC_MODE_LEVEL_AUTO &&
            tftSelectedLevelingMode != AC_MODE_LEVEL_MANUAL)
        {
          tftSelectedLevelingMode = AC_MODE_LEVEL_MANUAL;
        }
     #else
        unsigned char tftSelectedLevelingModeDummy;
        EEPROM_READ(tftSelectedLevelingModeDummy)
     #endif 

    _FIELD_TEST(tftSelectedOffsetZ);
     #if ENABLED(ANYCUBIC_CHIRON)
        EEPROM_READ(tftSelectedOffsetZ);
     #else
        EEPROM_READ(dummy);
     #endif 
    ...
    _FIELD_TEST(tftPowerLossCard);
     #if ENABLED(ANYCUBIC_CHIRON)
        EEPROM_READ(tftPowerLossCard);
     #else
        long tftPowerLossCardDummy;
        EEPROM_READ(tftPowerLossCardDummy)
     #endif 
...
void MarlinSettings::reset() {
  ...
  #if ENABLED(ANYCUBIC_CHIRON)
    tftSelectedLevelingMode = AC_MODE_LEVEL_MANUAL;
    tftSelectedOffsetZ      = Z_PROBE_OFFSET_FROM_EXTRUDER;
    tftPowerLossHeadE       = 0.0f;
    tftPowerLossHeadX       = 0.0f;
    tftPowerLossHeadY       = 0.0f;
    tftPowerLossHeadZ       = 0.0f;
    tftPowerLossCard        = 0;
  #endif
